...
- Otherwise, find renovate branches via `git branch -a | grep renovate` and also find the corresponding GitLab or GitHub pull request.
- Prefer the PRs where the pipeline did not fail.
- Ask the user which PR resp. branch to process.
...
Before merging:
- Check that the PR build is successful.
- There must not be any open discussions.
- Add a comment to the PR telling what you did. Use the prefix text "Generated by Claude".

Ask for confirmation to merge the PR.

- Merging several PRs concurrently sometimes fails. Give GitLab a bit of time to recover.
- If a PR cannot be merged due to conflicts, tell renovate to rebase/retry this PR. Then, leave this PR for now.
